...
export function VerificationScreen({ state, dispatch, barrierId, navigate }: { state: DemoState; dispatch: Dispatch<DemoAction>; barrierId: string; navigate: (path: string) => void }) {
  ...
  function decide(decision: 'accepted' | 'rejected' | 'additional_inspection') {
    const nextErrors = validateVerification({ decision, consented, testerRole, journeyId: journey!.id, accessRequirement: journey!.accessRequirement, definedTestConditions: conditions, reason }, { ownerRole: workOrder!.ownerRole, hasRepairEvidence: workOrder!.repairEvidence.length > 0 })
