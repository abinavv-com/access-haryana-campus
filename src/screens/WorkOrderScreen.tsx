...
const afterEvidence: EvidenceItem = {
  ...
  altText:
    "Illustrative after-repair example showing a clear gradual pathway; not the same real-world location as the before image.",
...
export function WorkOrderScreen({
  ...
  const [remedy, setRemedy] = useState(
    "Remove obstruction and restore the clear landing.",
  );
  ...
  function create() {
    ...
    dispatch({
      ...
      command: {
        ...
        reason:
          "Repair work assigned with owner, remedy, cost band and due date.",
        workOrder: {
  ...
  function submitEvidence() {
    dispatch({
      ...
      command: {
        ...
        actorPerspective: "facilities",
        reason:
          "Structured repair evidence submitted. This does not mean verified complete.",
  ...
  const continueButton =
    barrier.status === "awaiting_verification" ? (
      <button
        ...
        onClick={() => navigate(`/verification/${barrier.id}`)}
      >
        Continue to independent verification
      </button>
    ) : null;
  return (
    <div className="screen-stack">
      <section className="hero-panel">
        <p className="eyebrow">Repair work order · illustrative demo data</p>
        <h1>{barrier.title}</h1>
        <p>
          <strong>Status:</strong> {barrier.status.replaceAll("_", " ")}
        </p>
        {barrier.status === "awaiting_verification" ? (
          <p>
            <strong>Repair evidence submitted — awaiting verification.</strong>{" "}
            This does not mean verified complete; an independent bounded journey
            retest is required.
          </p>
        ) : null}
      </section>
      ...
      {!workOrder ? (
        <section>
          ...
          <div className="work-fields">
            <label className="field-stack">
              ...
              <input
                value={costBand}
                onChange={(event) => setCostBand(event.target.value)}
              />
            </label>
            <label className="field-stack">
              Due date
        ...
        <section>
          <h2>Assigned work</h2>
          <p>
            <strong>Assigned:</strong> {workOrder.ownerRole}
          </p>
          <p>{workOrder.remedy}</p>
          <p>
            <strong>Cost band:</strong> {workOrder.costBand}
          </p>
          <p>
            <strong>Due date:</strong>{" "}
            {new Date(`${workOrder.dueDate}T00:00:00`).toLocaleDateString(
              "en-IN",
          ...
          <div className="comparison-grid">
            ...
            <div>
              <h3>After repair</h3>
              <EvidenceImage
                evidence={workOrder.repairEvidence[0] ?? afterEvidence}
              />
              <p>
                Illustrative comparison only; post-repair measurement and
                independent retest remain required.
              </p>
            </div>
          </div>
          {barrier.status === "assigned" ? (
