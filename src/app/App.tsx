...
const preferenceKey='access-haryana-preferences'
...
export function App(){const[state,dispatch]=useReducer(demoReducer,undefined,createDemoFixture);const[role,setRole]=useState<SimulatedRole>('auditor');const initial=initialPreferences();const[reducedMotion,setReducedMotion]=useState(initial.reducedMotion===true);const[largeText,setLargeText]=useState(initial.largeText===true);const[highContrast,setHighContrast]=useState(initial.highContrast===true);useEffect(()=>{document.documentElement.dataset.reducedMotion=String(reducedMotion);document.documentElement.dataset.textSize=largeText?'large':'default';document.documentElement.dataset.highContrast=String(highContrast);localStorage.setItem(preferenceKey,JSON.stringify({reducedMotion,largeText,highContrast}))},[reducedMotion,largeText,highContrast]);return <AppShell state={state} dispatch={dispatch} role={role} onRoleChange={setRole} reducedMotion={reducedMotion} onReducedMotionChange={setReducedMotion} largeText={largeText} onLargeTextChange={setLargeText} highContrast={highContrast} onHighContrastChange={setHighContrast}><RouteContent/></AppShell>}
